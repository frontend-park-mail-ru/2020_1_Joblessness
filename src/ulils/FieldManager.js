...
export class FieldManager {
  /**
     ...
     * fieldsToValidate - объект из строк и/или объектов
     * Если элемент - строка, то создается стандартный ивент,
     * привязанный к элементу, у которого id равно значению строки
     * @example
     * "some_id" --> {id: "some_id", eventName: "change",
     * event: (e, page) => {this.field[key] = e.target.value}}
     * Если использован объект, то его поля аналогичны полям в примере,
     * Функция event должна возращать какое-то значение для
     * хранения его в поле field(передается при подтверждении)
     * @param {Object}apply
     * объект, содержащий описание ивента,
     * при вызове которого форма отправляется.
     * @example
     * {
     *     id: "some_button",
     *     eventName: "click",
     ...
     *     }
     * }
     * @param {string} applyFieldName - название поля,
     * в который выносится ивент apply
     * @param {function[]}warnings -  массив функций,
     * вызываемых при ошибке валидации.
        ...
        Object.keys(this.fieldsToValidate).map(
            (key) => {
              if (!this.field[key]) {
                const e =
                    document
                        .getElementById(this.fieldsToValidate[key].id);
