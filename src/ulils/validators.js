...
const validateString = validateXXX(s => typeof s === 'string', 'String');
const validateFunction = validateXXX(s => typeof s === 'function', 'Function');
...
to the pattern present inside the char class. And the number of matched chars must be from 0 to 19.
 */
const isLogin = login => /^(?=.*[A-Za-z0-9]$)[A-Za-z][A-Za-z\d.-]{5,19}$/.test(login);
export default {
    validateString,
    validateFunction,
    isSlavicName,
